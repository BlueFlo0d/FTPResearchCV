...
void unwrap_phase(cv::Mat &input,cv::Mat &output,cv::Mat &R){
    //uchar *mask=(uchar*)malloc(sizeof(uchar)*input.rows*input.cols);
    Mat mat_mask = Mat(input.rows,input.cols,CV_8UC1);
    report_reliability(R, mat_mask);
    uchar *mask = mat_mask.ptr(0);
    unwrap2D(input.ptr<float>(0), output.ptr<float>(0), mask, input.cols, input.rows, 0, 0);
    //free(mask);
}
